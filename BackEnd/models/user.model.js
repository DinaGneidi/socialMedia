...
const userSchema = new mongoose.Schema({
    ...
    friendsList: [{ type: String, trim: true }],
    tokens: [
...
userSchema.methods.toJSON = function() { //Disable(Password,...etc) from being display in Database         
...
userSchema.pre("save", async function() { //encrypt password before save
    const user = this
    if (user.isModified("password")) user.password = await bcrypt.hash(user.password, 10)
})
userSchema.virtual("userPosts", { //Relation Between (users) and (posts)
    ref: "Posts",
    localField: "_id",
    foreignField: "userID"
})
